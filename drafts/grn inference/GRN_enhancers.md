...
ATAC-seq and DNase-seq data can be analyzed for TF binding motifs, but this type of analysis is very prone to false positives. First, there are false positive motif occurrences: the lowest q-value I have ever seen when looking for [the Foxn1 motif](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5033077/) is about 0.4 :( . Second, there are false positives where the motif is correctly detected in a region of open chromatin, but the factor is not bound (CITATION NEEDED). This method does not give clean enough results to confirm individual TF-target pairs, so most practitioners use motif analysis by averaging signal across many loci. To infer incoming arrows, motif analysis could provide an initial boost, but I suspect the heavy lifting will have to be done by perturbation screens (LINK PERTURB-ATAC HERE) or correlation analysis (EXAMPLE? FANTOM5?) rather than direct binding.
